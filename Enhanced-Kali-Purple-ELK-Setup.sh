...
sudo /usr/share/elasticsearch/bin/elasticsearch-certutil ca -s -p system@123 -n elastic-stack-ca.p12
sudo /usr/share/elasticsearch/bin/elasticsearch-certutil cert \
  --ca elastic-stack-ca.p12 \
  --dns kali-purple.kali.purple,elastic.kali.purple,kali-purple \
  --out kibana-server.p12 -s -p system@123

sudo openssl pkcs12 -in kibana-server.p12 -out /etc/kibana/kibana-server.crt -clcerts -nokeys -password pass:system@123
sudo openssl pkcs12 -in kibana-server.p12 -out /etc/kibana/kibana-server.key -nocerts -nodes -password pass:system@123
