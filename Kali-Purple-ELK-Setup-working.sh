...
# Step 3: Convert Kibana server certificate (PKCS12) to PEM format
sudo openssl pkcs12 -in kibana-server.p12 -out /etc/kibana/kibana-server.crt -clcerts -nokeys

# Step 4: Extract the private key from the PKCS12 file
sudo openssl pkcs12 -in kibana-server.p12 -out /etc/kibana/kibana-server.key -nocerts -nodes

# Step 5: Change ownership and set appropriate permissions for the certificates
...
# Step 6: Configure Kibana to use SSL/TLS
